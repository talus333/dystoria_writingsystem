...
**App version:** v.658 (internal `APP_VERSION` still `2026.07.20.624`) · **Updated:** 2026-09-08 · _md5 `53a97b8c4df28069d774ac551357e933`, **written to the repo folder and verified on device**. `worker.js` md5 `c2161c9674113095942e7a27a81645d8`, unchanged._
...
| 27 | **Map sectioning ("Spaces")** — **read #48 first; the recommendation is to close this as superseded when #48's zones ship** | ☐ |
...
| 46 | **Stakes as one object** — 1a ✓ v.562 · 1a′ ✓ v.564 · 1b roster ✓ v.567, offer ✓ v.568–569, key identity ✓ v.570, the grid surfaces PLACED ✓ v.571; **the rest of 1b, portents + the stake column, and movement remain** | ◐ |
...
| 50 | **Events that move** — **phase 1 ✓ v.656** (`eventShift[key]={from,to}`: Before → After on the Event Builder, the card and the Scene Context row; section-free, rename-safe) · **phase 2 ✓ v.657** (`eventLinks=[{a,b,rel}]`, two verbs; the Event Builder's *Because of* block; the card's *Because of* / *Leads to*; Gap & Chekhov's certain finding for a cause that first appears after its effect, and for a circle — ordering read live, no index stored; the certain findings render even when the AI is down) · **phase 3 ✓ v.658** (`eventKnown[key]={acted,present,heard}` of being keys, one column per character; the Event Builder's *Who knows* block with the derived, never-stored *doesn't know* row; the card's four rows; rename carries the record and the keys inside it). **Left:** phase 4 the sweep UI (one field across every event, on the card-deck mechanism) and the domino walk. Note for phase 4: two unnamed-kind events share one key (`0|event|Event`). Three open questions in the design doc are Jeremy's (MICE open/close, `rel` vocabulary, whether the ledger wants a when) | ◐ |
...
**Remaining:** the rest of **1b** (point the eight surfaces at element keys instead of strings, retire the narrative layer's Stakes card in favour of a derived read, reconcile `START_LAYERS` — the start card offers four layers where Plan and Help list six), **portents + `COL_KINDS.stake`**, and **movement** (the ladder / the clock).
...
**⚠ `.pl-stake` carries `text-transform:lowercase`**, so a stake typed *The Harvest* reads *the harvest* on the bar and in the popup. Pre-existing and deliberate, but it sits against **"call a thing what the writer called it"** in two visible places. **Jeremy's call.**
**⚠ The open fork for 1b:** a stake element can be an *At Stake* noun (**The Harvest** — a thing that can be taken) or a *Conflict* word (**Against Fate** — the shape of the taking). They want different cards, and 1a treats them identically — right for two optional fields, wrong for the promotion.
