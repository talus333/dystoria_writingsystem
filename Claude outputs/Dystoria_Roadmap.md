...
| 69 | **The writing room** — Write-mode comfort, keys and questions. Design: `claude/Dystoria_Write_Room_Design.md` (2026-09-17). Four phases: **1 the room** (one shared prose size, 19px by default, with a scale bar and ⌘± · two background tones · ink brightness and colour) · **2 the keys** (shortcuts drawer · paired quotes and brackets · ⌘⌥1/2 and `#`/`##` for Section/Scene) · **3 the pointer** (click = soft target, double = sentence, triple = paragraph) · **4 the questions** (Refract · Questions in Write + the right sidebar Context · Questions · Keys) | ☐ |
...
- **Phase 1 · the room:** one prose size shared by Write and Revise (`--dy-prose-size`, **19px** by default — Write in a session is 22px today, Revise is 19px), a scale bar in `#fsHeader`, ⌘+ / ⌘− / ⌘0 (which block browser zoom **only while a session is active**), and agreement with Living Page (the slider sets Living Page's base size). **Two background tones:** the block being written in stays `--dy-room-1`; the other `.session-start` blocks and the gutter use `--dy-room-2`; the drawer tab uses the writing tone. **Ink:** five brightness steps × five tints, **each checked at ≥ 4.5:1 contrast**, Write only, ignored in e-ink.
...
- **Phase 4 · the questions:** **Refract · Questions** (⌘⌥R, ✦) — **exactly three questions, never prose**, in the selected `FEEDBACK_VOICES` voice, based on the passage + the surrounding section + `storyFacts()` for the Context elements + `goalContext()` + Drive / narrative layers, plus a **"missing" list worked out in code** (`elTextHit`, including aliases) so questions about what's absent can't be made up. With AI off, the missing list still appears as plain prompts. Pinned questions go to a **right sidebar (Context · Questions · Keys)**.
...
**Open for Jeremy:** keep Living Page's dimming now that there are two tones (#68 says ask first)? · the name (Refract · Questions / Ask the page) · move the whole Focus panel to the right, or show a copy? · double-click = sentence on by default? · the five ink tints · should pinned questions also show in Revise?
