...
| 69 | **The writing room** — Write-mode comfort, keys and questions. Design: `claude/Dystoria_Write_Room_Design.md` (2026-09-17). Four phases: **1 the room** (Living Page switched off, not deleted · one shared prose size, 19px by default, with a scale bar and ⌘± · two background tones · ink brightness and colour) · **2 the keys** (shortcuts drawer · paired quotes and brackets · ⌘⌥1/2 and `#`/`##` for Section/Scene) · **3 the pointer** (click = soft target, double = sentence, triple = paragraph) · **4 the questions** (Refract in Write + the right sidebar Context · Questions · Keys; pinned questions can be sent to Revise as notes). **All six open questions answered 2026-09-17** | ☐ |
...
- **Phase 1 · the room:** **Living Page's text growth and its fading of earlier text are switched off — hidden, not deleted** [Jeremy: _"not a feature until the groundwork is done"_]. One prose size shared by Write and Revise (`--dy-prose-size`, **19px** by default — Write in a session is 22px today, Revise is 19px), a scale bar in `#fsHeader`, ⌘+ / ⌘− / ⌘0 (which block browser zoom **only while a session is active**), and, with Living Page off, nothing else changes the size. **Two background tones:** the block being written in stays `--dy-room-1`; the other `.session-start` blocks and the gutter use `--dy-room-2`; the drawer tab uses the writing tone. **Ink:** five brightness steps × five tints, **each checked at ≥ 4.5:1 contrast**, Write only, ignored in e-ink.
