...
**App version:** v.665 (internal `APP_VERSION` still `2026.07.20.624`) · **Updated:** 2026-09-08 · _md5 `c11fd3e2244b3d44762c4b4b08705420`, **written to the repo folder and verified on device**. `worker.js` md5 `c2161c9674113095942e7a27a81645d8`, unchanged._
...
| 27 | **Map sectioning ("Spaces")** — **superseded by #48's zones (v.622+)**: many canvases with labelled boundaries inside each answered spatial containment with a fraction of the regression a 3×3 pane grid would carry. Closes when #48's bring-from lands; the reasoning stays in the Places design doc §8 | ☒ |
...
| 48 | **Places** — phase 0 ✓ v.602–v.606 (one editor: the map's Edit dialog is a route to the card; inline rename is the fast path) · phase 1 ✓ v.617 (Places, Scenes/Maps) · phase 2 ✓ v.618 (a board per scene, the carry-over offer) · phase 3 ✓ v.619–v.621 (maps as objects, nesting by pin) · zones ✓ v.622–v.632 · **zones offered into a new section ✓ v.664** · **the At Stake hook ✓ v.665** (an *At Stake* / *At risk* zone offers, under its pill, to name what each unanswered member stands to lose; the answer is a stake element with bearer + the containing Place as threat — offer, never apply). **Left:** §6 bring-from, both directions, as a copy with provenance (`w.fromMap`) — the last piece; #27 closes with it. *Named, not built:* changing an element's category has no door since the dialog retired (v.606). Supersedes #27 | ◐ |
