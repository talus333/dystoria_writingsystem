...
**App version:** v.774 (internal `APP_VERSION` `2026.07.20.697`) · **Updated:** 2026-09-23 (**Where next is CLOSED** — five phases in v.753–v.757, every §9 open question answered and built in v.758–v.759. v.760: the 60-second AI cut-off, the Plan drawer, Dictate as a held key. **v.761: the Stakes Builder** — a stake can finally be given a bearer and a threat from a builder, which is what the Conflict layer had been waiting on — **and the ten conflict axes, eight of which shared one lane, are mapped; `society` is its own lane now**. **v.762: the Library joins the dark chrome family; Checks and Research rejoin the light drawer rack**, reversing the standing "AI surfaces are dark" rule — the access gate, not the dress, says which panels need an account. **v.763: the writing dock** — Dictate moves to the top-left of the right gutter where no drawer covers it, with a **Music** button under it, which is the first way to reach the soundscapes from inside a session at all. **v.764: the old one-shot Where next is retired** — off the Focus drawer and both menus that opened it; the planner on the plot cards is the only Where next now, and it still writes the beat under the clock. **v.765: the Plot grid filter** — filter the grid by any elements linked to its cards, AND only, hiding what does not match (`Dystoria_Plot_Grid_Filter_Design.md`). **v.766: POV on the plot cards, phase 1** — every card opens with **POV · STAKES** on one line; the POV pill is the card's general-notes tab (General is gone), its eye opens a chooser, a departure from the book's narrator is gold, and a viewpoint lives on the SECTION (or scene), not the column. The element row is **In Scene · + add element**. Also: section hover white with a gold outline, Stakes pill regular size, closed-card chips no taller than their notes, and renames now follow plot-card links. **v.767: every card's notes tab wears the eye** — inert (no chooser) where the book names no single pair of eyes, and a book with no POV set reads *Notes · POV*. **v.768: POV phase 2, the marks** — the same eye beside every viewpoint character's name in the Characters list, above their head on the map, and listed on the Narrative layers POV row (book's narrator first, then anyone a section is told through, with a count). **Full 617→768 run green (141 harnesses), run for scope because the map render changed; next due at v.773.** **v.769: POV phase 3** — the Plot grid filter gains a **Viewpoint** group (*Hansel's eyes*, *Gretel's eyes*), ANDing with elements; and ✦ Summarize moves from the card header to the bottom-right of the note. **v.770: POV phase 4** — Where Next plans through the card's eyes (bound to what the viewpoint can see only in limited/close/first-person tellings), Explore from a card carries its viewpoint, and the Craft review finally reads `data.pov` and the per-section viewpoints. **v.771: HOTFIX — large stories were slow to load and unresponsive** (the POV readers re-walked the whole story per card per scene; now once per redraw — faster than before POV). Full 617→771 run green (144 harnesses); next due at v.776. **v.772: POV phase 5, the deterministic half** — a no-AI **Perspective check** in Refine's Checks drawer (first-person narration in a third-person book, a first-person section with no "I", a first-person narrator reporting someone else's thoughts), and Continuity flags a section told through someone not on its map. **v.773: four asks** — a closed Plot card shows its scenes as number tabs beside the title (one scene's notes at a time); Bonds pills no longer grow on hover (tap to select; link and × in a row beneath); the Notepad search works again (it had been overruled by an `!important` rule since at least v.150); the POV eye is placed from each figure's own height. **v.774:** the link glyph under a selected Bonds pill is small in its circle (`#sky svg{width:100%}` had stretched it). **Next: the two ✦ passes (head-hopping, what the viewpoint couldn't know) — WAITING on Jeremy using the deterministic ones on a real draft, per §9**; then open work elsewhere on the Board. _Was:_ POV phase 5 — the checks: a deterministic perspective pass and a Continuity invariant (viewpoint not on the section's map), then the ✦ passes (`Dystoria_POV_On_Plot_Cards_Design.md` §7.3–7.4)) · _md5 `64db71ce413ca59a228419d1ed34f16a`, **written to the repo folder and verified on device** (no `.git/index.lock`). `welcome_undaunted.html` md5 `714622923969ac23812a63f4f78420f0`. `worker.js` md5 `c2161c9674113095942e7a27a81645d8`, unchanged._
...
| 68 | **The writing session's start** — Continue brings the prose in cleanly and editable ✓ v.648; the Continue/Begin prompt indented with the paragraphs so the words land where it stood ✓ v.649; the prose on the page while the card asks, Start a new section clears it, and the `keepPrior` hold no longer leaks across sessions ✓ v.652. **Left:** the Living Page still DIMS earlier prose (its own effect, not the retired lock). **Ruled 2026-09-17: switched off (not deleted) during #69** until the groundwork is done | ◐ |
...
| 69 | **The writing room** — Write-mode comfort, keys and questions. Design: `claude/Dystoria_Write_Room_Design.md` (2026-09-17). Four phases: **1 the room** (Living Page switched off, not deleted · base prose 19px + a **view zoom** bar and ⌘± that scale the whole page proportionally · two background tones · ink brightness and colour) · **2 the keys** (shortcuts drawer · paired quotes and brackets · ⌘⌥1/2 and `#`/`##` for Section/Scene) · **3 the pointer** (click = soft target, double = sentence, triple = paragraph) · **4 the questions** (Refract in Write + the right sidebar Context · Questions · Keys; pinned questions can be sent to Revise as notes). **All six open questions answered 2026-09-17** | ☐ |
...
**69 · The writing room — Write-mode comfort, keys and questions** ☐
Design doc: `claude/Dystoria_Write_Room_Design.md`. [Jeremy, 2026-09-17] _"improve the write mode of the app to make it a more comfortable place to hang out."_ Twelve ideas, grouped into four phases. **Phases 1–3 need no AI.**
- **Phase 1 · the room:** **Living Page's text growth and its fading of earlier text are switched off — hidden, not deleted** [Jeremy: _"not a feature until the groundwork is done"_]. In-session prose goes to **19px** to match Revise (it's 22px today). **The slider is a VIEW ZOOM, not a text size** [Jeremy]: CSS `zoom` on the writing column (`--dy-view-zoom`, 70–180%, shared by Write and Revise, per viewer), with a bar in `#fsHeader` and ⌘+ / ⌘− / ⌘0 (which block browser zoom **only while prose is showing**). **Why `zoom` and not a font-size variable:** the Size menu (`edFmt('size')`) writes fixed inline px (14/18.5/24/32) into the prose, and those wouldn't scale with a variable — `zoom` keeps every size in proportion and writes nothing. **⚠ Test pointer maths (soft target, highlights, gutter, popovers) at 70% and 180%**; the fallback is `em` sizes plus a one-time conversion of existing spans. **Two background tones:** the block being written in stays `--dy-room-1`; the other `.session-start` blocks and the gutter use `--dy-room-2`; the drawer tab uses the writing tone. **Ink:** five brightness steps × five tints, **each checked at ≥ 4.5:1 contrast**, Write only, ignored in e-ink.
- **Phase 2 · the keys:** one keydown handler for the writing session, plus a **Keys** drawer (⌘/). Paired characters (`“”` `()` `[]` `{}` `«»`; they wrap a selection; a matching closer steps over; Backspace removes both; single quotes pair **only after a space**; nothing is paired while an input method is composing or during dictation). **New Section = ⌘⌥1 or `# `, New Scene = ⌘⌥2 or `## `.**
- **Phase 3 · the pointer:** a click places the caret **and** draws a *soft target* on the word (Highlight API — the same tool Find & Replace uses — so the text isn't touched). Clicking whitespace creates no target. Double-click = sentence, triple-click = paragraph, the session's gutter marker = the whole session. A "classic" setting brings back normal browser behaviour. Phones keep native selection.
- **Phase 4 · the questions:** **Refract** (⌘⌥R, ✦; the name stays for now [Jeremy]) — **exactly three questions, never prose**, in the selected `FEEDBACK_VOICES` voice, based on the passage + the surrounding section + `storyFacts()` for the Context elements + `goalContext()` + Drive / narrative layers, plus a **"missing" list worked out in code** (`elTextHit`, including aliases) so questions about what's absent can't be made up. With AI off, the missing list still appears as plain prompts. Pinned questions go to a **right sidebar (Context · Questions · Keys)**; **the whole left `#focusPanel` moves there as the Context tab** [Jeremy]; a pinned question can be **sent to Revise as an anchored note**.

**⚠ Four corrections to the original list** (all in the doc): **⌘H can't be used** (macOS hides the app before the page sees the key; browsers never pass the Windows key to a page) → ⌘⌥1/2 plus `#`/`##`. **"Super+Shift+>" is the same keystroke as "Super+>"** → colour uses ⌘⌥< / ⌘⌥>. **A real single-click word selection would be replaced by the next key typed** → the soft target; **its main test is "click a word, type a letter, the word is still there."** **Plain ⌘R reloads the page** → ⌘⌥R.
**⚠ Structure traps:** New Section **must go through `__planSectionSplit`** (one of the seven index-rewriting paths); New Scene is an ordinary `.scene-mark`. Pinned questions are stored at `npNotes['section:<i>||WQ:<id>']`, which the seven paths already carry (the #54 trick). Their `about` field is an element key, so the store **joins both key migrators and #18's fork list on the day it is written.** Moving Context from the left `#focusPanel` must first check what that panel was the last way into (+ Add element, Start, goal, images) at 1100px.
**Ruled [Jeremy, 2026-09-17]:** Living Page off for this push (not deleted) · the name is **Refract** · Context moves to the right-hand drawer · **double-click = sentence by default** · the five ink tints are approved · pinned questions → Revise notes. _This also answers #68's open question about Living Page's dimming, for now._
